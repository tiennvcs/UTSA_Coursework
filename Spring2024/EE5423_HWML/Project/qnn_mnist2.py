...
chosen_indices = np.random.choice(np.arange(len(x_train)), 90)
x_train = x_train[chosen_indices]
y_train = y_train[chosen_indices]
...
mnist2_qnn_train_time = time.time() - s_time

mnist2_qnn_train_score = simpleVQC.score(x_train, y_train)
print("QNN's training score: {}".format(mnist2_qnn_train_score))

mnist2_qnn_test_score = simpleVQC.score(x_test, y_test)
print("QNN's testing score: {}".format(mnist2_qnn_test_score))

print("QNN's training time: {} (seconds)".format(mnist2_qnn_train_time))